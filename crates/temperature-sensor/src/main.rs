...
struct Opt {
    ...
    /// Use the result generated by an external program.
    ///
    /// Specify a path to a TOML file, or `-` to read from stdin.
...
async fn main() -> Fallible<()> {
    ...
    let sensor_snapshot = opt
        .input
        .as_deref()
        .map(|path| {
            if path == Path::new("-") {
                return read_sensor_snapshot(std::io::stdin());
            }
    ...
    if let Some(sensor_snapshot) = sensor_snapshot {
        debug!(collected_at = %sensor_snapshot.collected_at, "loaded sensor snapshot");

        for entry in sensor_snapshot.readings {
            if sensor_values.iter().any(|v| v.name == entry.name) {
                warn!(name = %entry.name, "duplicate name; skipped");
                continue;
            }

            let Some(temperature) = entry.temperature_celsius else {
                warn!(name = %entry.name, "temperature_celsius is None");
                continue;
            };
...
fn parse_sensor_snapshot(input: &str) -> Fallible<SensorSnapshot> {
    let snapshot = toml::from_str::<SensorSnapshot>(input)?;
    ensure!(
...
fn read_sensor_snapshot(mut reader: impl Read) -> Fallible<SensorSnapshot> {
    let mut buffer = String::new();
    reader.read_to_string(&mut buffer)?;
    parse_sensor_snapshot(&buffer).context("failed to parse sensor snapshot")
}
...
"#;
    ...
    #[test]
    fn parse_sensor_snapshot_should_deserialize_correctly() {
        let actual = r#"
schema_version = 1
...
humidity_percent = 85.1
"#;
        let expected = SensorSnapshot {
            schema_version: 1,
            collected_at: Utc
                .with_ymd_and_hms(2014, 11, 28, 12, 0, 9)
                .unwrap()
                .with_timezone(&FixedOffset::from_str("+09:00").unwrap()),
            readings: vec![
                SensorReading {
                    name: "リビング（HomePod）".to_string(),
                    temperature_celsius: Some(25.2),
                    humidity_percent: Some(67.0),
                },
                SensorReading {
                    name: "ダイニング（HomePod）".to_string(),
                    temperature_celsius: Some(25.0),
                    humidity_percent: Some(85.1),
                },
            ],
        };
    ...
    #[test]
    fn parse_sensor_snapshot_should_return_error_when_missing_schema_version() {
        let actual = r#"
collected_at = "2014-11-28T21:00:09+09:00"
...
"#;
    ...
    #[test]
    fn sensor_snapshot_should_return_error_when_unsupported_schema_version() {
        let actual = r#"
schema_version = 2
collected_at = "2014-11-28T21:00:09+09:00"
...
"#;
    ...
    #[test]
    fn sensor_snapshot_should_return_error_when_missing_collected_at() {
        let actual = r#"
schema_version = 1
...
"#;
    ...
    #[test]
    fn sensor_snapshot_should_return_error_when_missing_name() {
        let actual = r#"
schema_version = 1
collected_at = "2014-11-28T21:00:09+09:00"
...
"#;
    ...
    #[test]
    fn sensor_snapshot_should_deserialize_correctly_when_missing_temperature_celsius_and_humidity_percent()
     {
        let actual = r#"
schema_version = 1
collected_at = "2014-11-28T21:00:09+09:00"
